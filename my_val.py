...
def run(
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        for si, pred in enumerate(preds):
            ...
            path, shape = mon.Path(paths[si]), shapes[si][0]
    ...
    if save_json and len(jdict):
        ...
        anno_json = str(mon.Path(data.get("path", "../coco")) / "annotations/instances_val2017.json")  # annotations json
        ...
        try:  # https://github.com/cocodataset/cocoapi/blob/master/PythonAPI/pycocoEvalDemo.ipynb
            ...
            if is_coco:
                eval.params.imgIds = [int(mon.Path(x).stem) for x in dataloader.dataset.im_files]  # image IDs to evaluate
            eval.evaluate()
            eval.accumulate()
            eval.summarize()
