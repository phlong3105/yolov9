...
def run(
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        for si, pred in enumerate(preds):
            ...
            path, shape = core.Path(paths[si]), shapes[si][0]
    ...
    if save_json and len(jdict):
        ...
        anno_json = str(core.Path(data.get("path", "../coco")) / "annotations/instances_val2017.json")  # annotations json
        ...
        try:  # https://github.com/cocodataset/cocoapi/blob/master/PythonAPI/pycocoEvalDemo.ipynb
            ...
            if is_coco:
                eval.params.imgIds = [int(core.Path(x).stem) for x in dataloader.dataset.im_files]  # image IDs to evaluate
            eval.evaluate()
            eval.accumulate()
            eval.summarize()
...
def main(
    ...
    if opt.task in ("train", "val", "test"):  # run normally
        ...
        if opt.task == "speed":  # speed benchmarks
            ...
            for opt.weights in weights:
                f = f"study_{core.Path(opt.data).stem}_{core.Path(opt.weights).stem}.txt"  # filename to save to
